...
A network of Source Verification services, led by Runtime Verification. An hub for verification data, built on Stellar Registry.
...
We will build a network of verification services and the on-chain, public-infrastructure hub to integrate them all and surface their information. 
...
2. Stellar Registry data hub: Stellar Registry already allows adding names and semantic versions to Stellar smart contracts, turning the inscrutable trail of wasm hash uploads into a meaningful (and instrumentable) narrative. Once `(source code, build image)` information is added to the wasm metadata, Registry will also expose this data via its contract calls and UI.
...
## Verifier added to Registry's allowlist

The Registry Security Council is a group of known individuals & organizations using a governance process build on [Tansu](https://tansu.dev/).

```mermaid
sequenceDiagram
  participant Verifier as Source Verification Service
  participant Council as Registry Security Council
  participant Registry as Stellar Registry Contract

  Verifier->>Council: add_verifier(<br>name, address,<br>website, proposal)<br>
  Council->>Council: human security council review (1-7 days)
  Council->>Registry: if approved:<br>add_source_verifier_to_allowlist(<br>address, name, website)<br>
```

## Consumer gets verifiers

"Consumer" is any external service, dapp, or user, including [stellar.rgstry.xyz](https://rgstry.xyz)

This endpoint will show only verifiers on the allowlist. If one verifier has been approved and added following the process shown above, this will return an array (aka `Vec`) of length 1.
...
sequenceDiagram
  ...
  Consumer->>Registry: get_verifiers()
  Registry->>Consumer: Vec<{<br>address: Address, name: String, website: String }><br>
```

## Verifier removed from allowlist

Any "Good Citizen" (security council member, ecosystem participant, interested party) can open a proposal to remove a verifier from Registry's allowlist.

```mermaid
sequenceDiagram
  actor Citizen as Good Citizen
  participant Council as Stellar Registry Security Council
  participant Registry as Stellar Registry Contract

  Citizen->>Council: remove_verifier(<br>address, proposal)<br>
  Council->>Council: human security council review (1-7 days)
  Council->>Registry: if approved:<br>remove_source_verifier(address)<br>
```

## Consumer gets all verifiers

...including those removed from the allowlist.
...
sequenceDiagram
  ...
  Consumer->>Registry: get_all_verifiers()
  Registry->>Consumer: Vec<{<br>address: Address,<br>name: String,<br>website: String,<br>is_active: bool }><br>
```

## Main Source Verifier server process loop

Source Verifier Services will run this loop more-or-less continuously. They can choose to do this however they wish, such as by using an indexing service or polling the blockchain every N seconds/minutes.

```mermaid
sequenceDiagram
  participant Verifier as Source Verifier
  participant Stellar as Stellar Blockchain
  participant Registry as Registry Smart Contract

  Verifier->>Stellar: check for new wasms uploaded<br>with (source, image) metadata, or<br>for (source, image) data added to<br>Registry wasm data<br>
  Stellar->>Verifier: (wasm_hash, source, image) to verify

  Verifier->>Verifier: is image trusted?
  Verifier->>Verifier: is source reachable & well-formed?
  Verifier->>Verifier: `stellar contract verify`
  Verifier->>Registry: source_is_valid(<br>wasm_hash, result (true/false),<br>evidence_uri, generic options object)<br>
```

Where:

- `evidence_uri` is an off-chain signed per-operator detail (an optional link to the verifier's page documenting their verification)
- `generic options object` is a generic object allowing free-form data to be sent. This will be ignored by Registry for now, but will later allow the interface to be expanded without breaking changes.

## Consumer gets contract verifications

All verifications will be available via an RPC call directly to the Registry contract, providing the greatest possible data permanence and interoperability. All verifications are returned, surfacing disagreements and exposing history. Verifications are also summarized, providing quick high-level information about verifier sentiment. (If all verifiers agree, then the summary is probably sufficient. If they disagree, the full verification list/history becomes useful.)
...
sequenceDiagram
  ...
  Consumer->>Registry: get_verifications(wasm_hash)
  Registry->>Consumer: Vec<{<br>summary: { verifiers_saying_valid: u32, verifiers_reporting: u32 },<br>history: Vec<{ verifier_name: String, verified_at: Timestamp,<br>verification_result: bool, evidence_uri: Option<String> }><br>}><br>
```
